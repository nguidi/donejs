    ...
    function(){
        ...
        can.Control("Filtro",{
            ...
            insertCascada: function(level,filterData,from) {
                ...
                element.append(
                                can.view('view/cascada/menu.ejs',
                                    {
                                        filter_options: this.getFilterMenu(filterData)
                                    }
                                )
                              )
            },
            ...
            insert: function(elemClass,filterData) {
                ...
                $.each(filterData,function(index,filter){
                    self.element.find('div.'+elemClass+':last')
                        .append(
            ...
            add: function(options) {
                ...
                $.each(fromArray,function(i, from){
                    var level = self.getLevelFrom(from)
                    var element = self.addFilterFrom(level+1,from)
                    element.append(
                                can.view('view/cascada/'+options.type+'.ejs',
                                    {
                                        filter_options: self.getFilterMenu(options.filterData)
                                    }
                                )
                           )
                    if (options.type == 'compare')
                        self.addFilterFrom(level+2,from).append(can.view('view/input.ejs')) 
                })
            },
            
            getFilterMenu: function(filterData) {
                return (isUndefined(filterData)) ? undefined : $.map(filterData,function(filter,i){
                    return {
                        label: filter.title,
            ...
            addFilterFrom: function(level,from){
                ...
                return this.element.find('div[filter-level="'+level+'"] div[from="'+stringToRE(from)+'"]')
            },

            getLevel: function(element) {
                if (this.options.filterType == 'cascada')
            ...
            getFilterObject: function(element) {
                return {
                    ...
                    value: (element.is('input, select')) ? element.val() : $.trim(element.text()),
            ...
            'button.filtrar click': function() {
                var self = this
                this.element.find('input, select').each(function(i,e){
                    if (($(e).is('input') && $(e).val().length != 0) || ($(e).is('select') && $(e).val() != -1))
                        self.options.filterPath.push(self.getFilterObject($(e)))
                })
                this.options.filterFunction(this.options.filterPath)
            }
        }
